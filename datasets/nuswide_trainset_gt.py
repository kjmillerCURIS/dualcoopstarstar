...
class nuswide_partial(DatasetBase):
    def __init__(self, cfg):
        ...
        self.im_name_list_test = self.read_name_list(join(self.dataset_dir, 'TestImagelist_filtered.txt'), False)
        print('NUS-WIDE total {} images, test {} images. '.format(len(self.im_name_list), len(self.im_name_list_test)))

        path_labels = os.path.join(self.dataset_dir, 'Tag81')
        ...
        '''
        train_labels = np.loadtxt(os.path.join(path_labels, 'TrainTags81_filtered.txt'))

        train_labels = train_labels*2-1

        
        train_labels_partial = changeLabelProportion(train_labels, partial_prob)
        ...
        test_labels = np.loadtxt(os.path.join(path_labels, 'TestTags81_filtered.txt'))
