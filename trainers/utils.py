...
class AsymmetricLoss_softmax_partial(nn.Module):
    def __init__(self, gamma_neg=4, gamma_pos=1, clip=0.05, eps=1e-8, disable_torch_grad_focal_loss=True):
        super(AsymmetricLoss_softmax_partial, self).__init__()
    ...
    def forward(self, x, y, thresh_pos=0.9, thresh_neg=-0.9, if_partial=True, verbose=False):
        """"
        Parameters
        ----------
        x: input logits
        ...
        if verbose:
            print(-loss.cpu().numpy()[0])

        return -loss.sum()*0.02 #/ x.shape[0] if if_partial else -loss.mean()


def dualcoop_softmax_loss(inputs, inputs_g, targets, verbose=False):
    """
    using official ASL loss.
    """
    loss_fun = AsymmetricLoss_softmax_partial(gamma_neg=2, gamma_pos=1, clip=0.05)

    return loss_fun(inputs, targets, thresh_pos=0.9, thresh_neg=-0.9, verbose=verbose) # + loss_fun(inputs_g, targets)   
