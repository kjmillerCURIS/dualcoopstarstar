...
class Caption_tri_wta_soft_pseudolabel_ternary_cooccurrence(TrainerX):
    ...
    def load_ternary_cooccurrence_mat(self):
        assert(self.cfg.TRAIN.TERNARY_COOCCURRENCE_LOSS_TYPE == 'prob_stopgrad_logit')
    ...
    def compute_cooccurrence_loss(self, output):
        ...
        logits = torch.unsqueeze(output, dim=-1) #(batch_size, num_classes, 1)
        probs = torch.sigmoid(logits)
        probs = probs.detach()
        product = 0.5 * probs @ torch.permute(logits, (0,2,1)) + 0.5 * logits @ torch.permute(probs, (0,2,1))
        assert(product.shape == (logits.shape[0], self.ternary_cooccurrence_mat.shape[0], self.ternary_cooccurrence_mat.shape[1]))
        cooccurrence_loss = torch.mean(torch.sum(torch.unsqueeze(self.ternary_cooccurrence_mat, dim=0) * product, dim=(1,2)))
        return cooccurrence_loss

    #output, pseudolabel_logits, pseudolabel_weights all have shape (batch_size, num_classes)
    #will return a single scalar
    def compute_loss(self, output, pseudolabel_logits, pseudolabel_weights, gt_labels=None):
        assert(len(output.shape) == 2)
        ...
        cooccurrence_loss = self.compute_cooccurrence_loss(output) #cooccurrence_loss will already be weighted by alpha
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            output_g, output, _, _ = self.model(image)
            loss, erm_loss, cooccurrence_loss = self.compute_loss(output, pseudolabel_logits, pseudolabel_weights, gt_labels=gt_labels)
