...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        temperature_pos = torch.tensor(3, dtype=dtype)  # 50
        self.temperature_pos = nn.Parameter(temperature_pos)
        temperature_neg = torch.tensor(3, dtype=dtype)  # 50
        self.temperature_neg = nn.Parameter(temperature_neg)
        temperature_wta = torch.tensor(5.3, dtype=dtype)  # 50
    ...
    def forward(self, neg_prompt_wcls=True):
        ...
        return prompts, prompts_pos, prompts_neg, self.temperature_pos, self.temperature_neg, self.temperature_wta, self.spatial_T
...
class DenseCLIP(nn.Module):
    ...
    def forward(self, image, norm=True):
        ...
        prompts, prompts_pos, prompts_neg, temperature_pos, temperature_neg, temperature_wta, spatial_T = self.prompt_learner()
        ...
        logit_scale_pos = temperature_pos.exp() 
        logit_scale_neg = temperature_neg.exp() 
        logit_scale_wta = temperature_wta.exp() 
...
class Caption_tri_wta_soft_ada(TrainerX):
    @torch.no_grad()
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output_g = output_g[:,1,:] - output_g[:,0,:] 
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            if self.cfg.TRAIN.LOSSFUNC == 'sigmoid':
                ...
                loss = dualcoop_softmax_loss(output, output_g, label)
