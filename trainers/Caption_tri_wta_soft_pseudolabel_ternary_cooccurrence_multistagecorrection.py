...
class Caption_tri_wta_soft_pseudolabel_ternary_cooccurrence_multistagecorrection(TrainerX):
    ...
    def build_model(self):
        ...
        self.correction_epochs = [int(x) for x in self.cfg.TRAIN.MULTISTAGECORRECTION_EPOCHS.strip('_').split('_')]
    ...
    def before_train(self):
        ...
        self.pred_logits_buffer = self.pseudolabel_logits.detach().clone()
    ...
    def after_epoch(self):
        super().after_epoch()
        assert(self.cfg.TRAIN.SKIP_PSEUDOLABEL_UPDATE_IN_CODE)
        if self.epoch in self.correction_epochs:
            corrected_pred_logits = _correct_initial_pseudolabels(self.pred_logits_buffer, self.ternary_cooccurrence_mat, self.cfg.TRAIN.TERNARY_COOCCURRENCE_LOSS_TYPE)
            with torch.no_grad():
                mu = self.cfg.TRAIN.MULTISTAGECORRECTION_MU
                old_probs = torch.sigmoid(self.pseudolabel_logits)
                new_probs = torch.sigmoid(corrected_pred_logits)
                probs = mu * old_probs + (1 - mu) * new_probs
                self.pseudolabel_logits = torch.log(probs) - torch.log(1 - probs)

            checkpoint_filename = osp.join(self.output_dir, 'pseudolabels.pth.tar-corrected_epoch%03d'%(self.epoch + 1))
            checkpoint = {'epoch' : self.epoch + 1, 'pseudolabel_logits' : self.pseudolabel_logits, 'pseudolabel_weights' : self.pseudolabel_weights}
            torch.save(checkpoint, checkpoint_filename)
    ...
    def forward_backward(self, batch):
        image, pseudolabel_logits, pseudolabel_weights, idx = self.parse_batch_train(batch)
        ...
        if prec == "amp":
            ...
            self.pred_logits_buffer[idx,:] = output.detach().clone()
    ...
    def parse_batch_train(self, batch):
        ...
        return input, pseudolabel_logits, pseudolabel_weights, idx
