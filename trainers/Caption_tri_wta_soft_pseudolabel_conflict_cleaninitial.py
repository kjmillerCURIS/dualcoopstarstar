...
from .conflict_cleaner import clean_initial_pseudolabels as _clean_initial_pseudolabels
_tokenizer = _Tokenizer()


CONFLICT_MATRIX_DIR = '../vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/conflict_matrices'
...
class Caption_tri_wta_soft_pseudolabel_conflict_cleaninitial(TrainerX):
    ...
    def build_model(self):
        ...
        if self.cfg.TRAIN.CONFIDENTONLY:
            self.conflict_matrix = None
        else:
            self.load_conflict_matrix()

    #expect it to be binary and triu
    #make it symmetric
    def load_conflict_matrix(self):
        conflict_matrix_filename = os.path.join(CONFLICT_MATRIX_DIR, self.cfg.DATASET.NAME, self.cfg.TRAIN.CONFLICT_MATRIX_NAME + '.pkl')
        with open(conflict_matrix_filename, 'rb') as f:
            conflict_matrix = pickle.load(f)

        assert(conflict_matrix['classnames'] == self.dm.dataset.classnames)
        conflict_matrix = conflict_matrix['mat']
        assert(conflict_matrix.shape == (len(self.dm.dataset.classnames), len(self.dm.dataset.classnames)))
        assert(np.all((conflict_matrix == 0) | (conflict_matrix == 1)))
        assert(np.allclose(conflict_matrix, np.triu(conflict_matrix, k=1)))
        conflict_matrix = conflict_matrix + conflict_matrix.T
        self.conflict_matrix = torch.tensor(conflict_matrix, device='cuda')
        if self.cfg.TRAIN.USE_GTMARGS:
            with open(GTMARGS_FILENAME_DICT[self.cfg.DATASET.NAME], 'rb') as f:
                gtmargs = pickle.load(f)

            assert(gtmargs['classnames'] == self.dm.dataset.classnames)
            gtmargs = gtmargs['mat']
            assert(gtmargs.shape == (len(self.dm.dataset.classnames),))
            assert(np.all((gtmargs > 0) & (gtmargs < 1)))
            self.gtmargs = torch.tensor(gtmargs, device='cuda', dtype=torch.float32)
        else:
            self.gtmargs = None
    ...
    def before_train(self):
        ...
        self.clean_initial_pseudolabels()

    def clean_initial_pseudolabels(self):
        self.pseudolabel_weights = _clean_initial_pseudolabels(self.pseudolabel_logits, self.conflict_matrix, self.cfg, gtmargs=self.gtmargs)
        ...
        checkpoint_filename = osp.join(self.output_dir, 'pseudolabels.pth.tar-init_cleaned')
