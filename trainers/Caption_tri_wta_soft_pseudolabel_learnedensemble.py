...
class PromptLearner(nn.Module):
    ...
    def __init__(self, cfg, classname_lists, clip_model):
        ...
        assert(all([len(l) == ensemble_size for l in classname_list]))
...
class DenseCLIP(nn.Module):
    ...
    def forward(self, image, norm=True):
        ...
        text_features_pos = torch.bmm(text_features.permute(0,2,1), F.softmax(ensemble_logits_pos, dim=1).unsqueeze(-1)).squeeze(-1)
        text_features_neg = torch.bmm(text_features.permute(0,2,1), F.softmax(ensemble_logits_neg, dim=1).unsqueeze(-1)).squeeze(-1)
...
class Caption_tri_wta_soft_pseudolabel_learnedensemble(TrainerX):
    ...
    def build_model(self):
        ...
        self.model = DenseCLIP(cfg, classname_lists, clip_model)
    ...
    def get_initializing_text_embeddings(self):
        ...
        classnames = self.dm.dataset.classnames
        classname_lists = get_classname_lists(classnames, self.cfg)
