...
TRAINER=Caption_tri_wta_soft_pseudolabel_conflict_cleaninitial
...
do
    ...
    python train_caption_pseudolabel_conflict_cleaninitial.py \
    --root ${DATA} \
    --seed ${SEED} \
    --trainer ${TRAINER} \
    ...
    TRAIN.CONFLICT_MATRIX_NAME ${CONFLICT_MATRIX_NAME} \
    TRAIN.USE_GTMARGS 1 \
    TRAIN.CLEAN_P ${CLEAN_P} \
    TRAIN.CLEAN_Q ${CLEAN_Q} \
    TRAIN.CLEAN_R ${CLEAN_R}
done
