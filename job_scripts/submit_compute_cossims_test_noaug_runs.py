...
DEBUG = False
SCRIPT_NAME = 'generic_compute_cossims_test_noaug.sh'
...
def submit_compute_cossims_test_noaug_runs():
    for dataset_name in ['nuswideTheirVersion_partial']: #['COCO2014_partial']: #['VOC2007_partial']:
        for model_type in MODEL_TYPE_LIST:
            if model_type == 'RN50':
                continue

            for single_probe_type in ['ensemble_80']: #SINGLE_PROBE_TYPE_LIST:
                job_name = 'compute_cossims_test_noaug_%s_%s_%s'%(dataset_name, model_type, single_probe_type)
...
if __name__ == '__main__':
    submit_compute_cossims_test_noaug_runs()
