...
DEBUG = False
SCRIPT_NAME = 'generic_second_max_experiments_TaI-DPT.sh'
DATASET_NAME_LIST = ['VOC2007_partial', 'COCO2014_partial', 'nuswideTheirVersion_partial']
SEED_LIST = [1, 2, 3]


def submit_second_max_experiments_taidpt_runs():
    for seed in SEED_LIST:
        for dataset_name in DATASET_NAME_LIST:
            for is_strong in [0,1]:
                for use_rowcalibbase in [0,1]:
                    job_name = 'secondmax_TaI-DPT_%s_rowcalibbase%d_strong%d_seed%d'%(dataset_name.split('_')[0], use_rowcalibbase, is_strong, seed)
...
if __name__ == '__main__':
    submit_second_max_experiments_taidpt_runs()
