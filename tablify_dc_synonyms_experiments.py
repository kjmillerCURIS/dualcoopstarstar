...
TABLE_FILENAME = os.path.join(OUTPUT_BASE_DIR, '../plots_and_tables/dc_synonyms_3.27.2024.csv')
BETTER_POLYSEMY_LIST = [0, 1]
APPEND_CLASS_LIST = [0, 1]
ORIG_CLASSNAMES_INIT_LIST = [0, 1]
THREE_SEP_LIST = [0, 1]
...
def get_results_dir(better_polysemy, append_class, orig_classnames_init, three_sep):
    llm_name = 'synonyms8' + ('_betterPolysemy' if better_polysemy else '') + ('_appendOrigClassname' if append_class else '')
    job_id = 'frozen_pseudolabel_wskip_%s_originit%d_threesep%d_coco_seed1'%(llm_name, orig_classnames_init, three_sep)
    return os.path.join(OUTPUT_BASE_DIR, job_id, 'Caption_tri_wta_soft_pseudolabel_learnedensemble/rn101/nctx21_cscTrue_ctpend/seed1/results')
...
def tablify_dc_synonyms_experiments():
    f = open(TABLE_FILENAME, 'w')
    f.write('method,synonym strategy,append orig classname?,pseudolabel init strategy,three separate ensembles?,best_val_mAP,best_epoch,last_saved_epoch\n')
    baseline_results_dir = get_baseline_results_dir()
    baseline_best_val_mAP, baseline_best_epoch, baseline_last_saved_epoch = process_results_dir(baseline_results_dir)
    f.write('baseline,N/A,N/A,use orig classnames only,N/A,%f,%d,%d\n'%(baseline_best_val_mAP, baseline_best_epoch, baseline_last_saved_epoch))
        ...
        for append_class in APPEND_CLASS_LIST:
            for orig_classnames_init in ORIG_CLASSNAMES_INIT_LIST:
                for three_sep in THREE_SEP_LIST:
                    results_dir = get_results_dir(better_polysemy, append_class, orig_classnames_init, three_sep)
                    if not os.path.exists(results_dir):
                        print('MISSING results for: (%d, %d, %d, %d)'%(better_polysemy, append_class, orig_classnames_init, three_sep))
                        continue
                    ...
                    f.write('learned synonym ensemble,%s,%s,%s,%s,%f,%d,%d\n'%(synonym_strategy, yesno(append_class), pseudolabel_init_strategy, yesno(three_sep), best_val_mAP, best_epoch, last_saved_epoch))

    f.close()


if __name__ == '__main__':
    tablify_dc_synonyms_experiments()
