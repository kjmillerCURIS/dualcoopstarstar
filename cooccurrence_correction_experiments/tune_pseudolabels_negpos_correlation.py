...
RESULTS_PREFIX = os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/mscoco_training-oracle-negpos-correlation-results/mscoco_training-oracle-negpos-correlation-results')
...
def tune_pseudolabels_negpos_correlation(alpha, epsilon, beta, zeta):
    alpha = float(alpha)
    epsilon = float(epsilon)
    beta = float(beta)
    zeta = float(zeta)

    p = {'alpha' : alpha, 'epsilon' : epsilon, 'beta' : beta, 'zeta' : zeta, 'negpos_cost_type' : 'correlation'}
...
def usage():
    print('Usage: python tune_pseudolabels_negpos_correlation.py <alpha> <epsilon> <beta> <zeta>')


if __name__ == '__main__':
    tune_pseudolabels_negpos_correlation(*(sys.argv[1:]))
