...
PSEUDOLABEL_LOGITS_FILENAME_DICT = {'COCO2014_partial' : os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/mscoco_training_init_pseudolabel_logits.pkl'), 'nuswide_partial' : os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/nuswide_training_init_pseudolabel_logits.pkl'), 'VOC2007_partial' : os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/voc2007_training_init_pseudolabel_logits.pkl')}
...
def compute_pseudolabel_logits(images, clip_model, text_embeddings):
    with torch.no_grad():
        images = images.cuda()
...
def compute_initial_pseudolabels(dataset_name):
    ...
    for batch in tqdm(dataloader):
        ...
        logits = compute_pseudolabel_logits(images, clip_model, text_embeddings)
        for impath, vec in zip(impaths, logits):
            assert(impath not in pseudolabel_logits)
            pseudolabel_logits[impath] = vec

    with open(PSEUDOLABEL_LOGITS_FILENAME_DICT[dataset_name], 'wb') as f:
        pickle.dump(pseudolabel_logits, f)


def usage():
    print('Usage: python compute_initial_pseudolabels.py <dataset_name>')


if __name__ == '__main__':
    compute_initial_pseudolabels(*(sys.argv[1:]))
