...
WARMUP_LR = 1e-8 #1e-5
MOMENTUM = 0.0
OUTPUT_DIR = os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/tune_pseudolabels_metric_matching_clamped_results')
...
def compute_metric_loss(pseudolabel_probs, d_metric, alpha, clamp_max):
    d_hat_metric = compute_d_hat_metric(pseudolabel_probs)
    diffs = torch.triu(torch.clamp(d_hat_metric, max=clamp_max) - torch.clamp(d_metric, max=clamp_max), diagonal=1)
    return alpha * torch.sum(torch.square(diffs))


def compute_total_loss(pseudolabel_probs, d_metric, alpha, clamp_max, init_pseudolabel_probs):
    metric_loss = compute_metric_loss(pseudolabel_probs, d_metric, alpha, clamp_max)
...
def one_optimization_step(pseudolabel_probs, d_metric, alpha, clamp_max, init_pseudolabel_probs, optimizer):
    optimizer.zero_grad()
    loss, proximal_loss, metric_loss = compute_total_loss(pseudolabel_probs, d_metric, alpha, clamp_max, init_pseudolabel_probs)
    loss.backward()
...
def make_results_dict(mAP, tuned_pseudolabel_probs, losses, proximal_losses, metric_losses, gradnorms, gradmaxes, init_metric_loss, alpha, clamp_max, dataset_name):
    params = {'alpha' : alpha, 'clamp_max' : clamp_max, 'metric_denominator_floor' : METRIC_DENOMINATOR_FLOOR, 'bce_floor' : BCE_FLOOR, 'optimizer' : OPTIMIZER, 'scheduler' : SCHEDULER, 'num_epochs' : NUM_EPOCHS, 'num_steps_per_epoch' : NUM_STEPS_PER_EPOCH, 'lr' : LR, 'warmup_lr' : WARMUP_LR, 'momentum' : MOMENTUM}
...
def get_results_dict_filename(dataset_name, alpha, clamp_max):
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    return os.path.join(OUTPUT_DIR, 'tune_pseudolabels_metric_matching_clamped_%s_alpha%s_clamp_max%s_bce_floor%s_lr%s_warmup_lr%s_num_epochs%d.pth'%(dataset_name.split('_')[0], str(alpha), str(clamp_max), str(BCE_FLOOR), str(LR), str(WARMUP_LR), NUM_EPOCHS))
...
def tune_pseudolabels_metric_matching_clamped(dataset_name, alpha, clamp_max):
    alpha = float(alpha)
    ...
    with torch.no_grad():
        init_metric_loss = compute_metric_loss(init_pseudolabel_probs, d_metric, alpha, clamp_max)
    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        for _ in range(NUM_STEPS_PER_EPOCH):
            loss, proximal_loss, metric_loss, gradnorm, gradmax = one_optimization_step(pseudolabel_probs, d_metric, alpha, clamp_max, init_pseudolabel_probs, optimizer)
    ...
    print('mAP(alpha=%s, clamp_max=%s, bce_floor=%s, lr=%s, warmup_lr=%s, num_epochs=%d) = %f'%(str(alpha), str(clamp_max), str(BCE_FLOOR), str(LR), str(WARMUP_LR), NUM_EPOCHS, mAP))

    results_dict = make_results_dict(mAP, tuned_pseudolabel_probs, losses, proximal_losses, metric_losses, gradnorms, gradmaxes, init_metric_loss, alpha, clamp_max, dataset_name)
    results_dict_filename = get_results_dict_filename(dataset_name, alpha, clamp_max)
    torch.save(results_dict, results_dict_filename)


def usage():
    print('Usage: python tune_pseudolabels_metric_matching_clamped.py <dataset_name> <alpha> <clamp_max>')


if __name__ == '__main__':
    tune_pseudolabels_metric_matching_clamped(*(sys.argv[1:]))
