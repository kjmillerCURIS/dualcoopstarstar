...
RESULTS_PREFIX = os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/mscoco_training-oracle-negpos-prob_stopgrad_dampedlogit-results/mscoco_training-oracle-negpos-prob_stopgrad_dampledlogit-results')
...
def tune_pseudolabels_negpos_prob_stopgrad_dampedlogit(alpha, epsilon, beta, zeta, rho):
    alpha = float(alpha)
    ...
    p = {'alpha' : alpha, 'epsilon' : epsilon, 'beta' : beta, 'zeta' : zeta, 'rho' : rho, 'negpos_cost_type' : 'prob_stopgrad_dampedlogit'}
    ...
    results_filename = RESULTS_PREFIX + '-alpha%f-epsilon%f-beta%f-zeta%f-rho%f.pkl'%(alpha, epsilon, beta, zeta, rho)
    ...
    print('mAP(%f, %f, %f, %f, %f) = %f'%(alpha, epsilon, beta, zeta, rho, mAP))


def usage():
    print('Usage: python tune_pseudolabels_negpos_prob_stopgrad_dampedlogit.py <alpha> <epsilon> <beta> <zeta> <rho>')


if __name__ == '__main__':
    tune_pseudolabels_negpos_prob_stopgrad_dampedlogit(*(sys.argv[1:]))
