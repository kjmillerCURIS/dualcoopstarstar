...
WARMUP_LR = 1e-8 #1e-5
MOMENTUM = 0.0
OUTPUT_DIR = os.path.expanduser('~/data/vislang-domain-exploration-data/dualcoopstarstar-data/cooccurrence_correction_experiments/tune_pseudolabels_metric_matching_L2proximal_results')
...
def compute_proximal_L2_loss(pseudolabel_probs, init_pseudolabel_probs):
    assert(len(pseudolabel_probs.shape) == 2)
    assert(len(init_pseudolabel_probs.shape) == 2)
    return torch.sum(torch.sum(torch.square(pseudolabel_probs - init_pseudolabel_probs), dim=1))
...
def compute_total_loss(pseudolabel_probs, d_metric, alpha, init_pseudolabel_probs):
    ...
    proximal_L2_loss = compute_proximal_L2_loss(pseudolabel_probs, init_pseudolabel_probs)
    total_loss = metric_loss + proximal_L2_loss
    return total_loss, proximal_L2_loss, metric_loss
...
def get_results_dict_filename(dataset_name, alpha):
    ...
    return os.path.join(OUTPUT_DIR, 'tune_pseudolabels_metric_matching_L2proximal_%s_alpha%s_bce_floor%s_lr%s_warmup_lr%s_num_epochs%d.pth'%(dataset_name.split('_')[0], str(alpha), str(BCE_FLOOR), str(LR), str(WARMUP_LR), NUM_EPOCHS))
...
def tune_pseudolabels_metric_matching_L2proximal(dataset_name, alpha):
    alpha = float(alpha)
...
def usage():
    print('Usage: python tune_pseudolabels_metric_matching_L2proximal.py <dataset_name> <alpha>')


if __name__ == '__main__':
    tune_pseudolabels_metric_matching_L2proximal(*(sys.argv[1:]))
