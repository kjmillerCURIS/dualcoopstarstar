...
from result_utils_11_21_23 import extract_mAPs_given_hparams, extract_mAPs_given_runID, extract_random_chance_mAPs, extract_zsclip_mAPs
...
def plot_results_11_21_23_adjust_logits_0():
    plt.clf()
        ...
        for use_bias in [0,1]:
            for bandwidth in [0.1, 0.2, 0.4]:
                for stepsize in [0.0625, 0.125, 0.25, 0.5, 1.0]:
                    ...
                    ret = extract_mAPs_given_hparams(do_adjust_logits, use_bias, bandwidth, stepsize)
                    if ret is None:
                        continue
    ...
    zsclip_mAP = extract_zsclip_mAPs()
    plt.plot(plt.xlim(), [zsclip_mAP, zsclip_mAP], color='k', linestyle='dashed', label='zsclip (ensemble)')
#    random_chance_mAP = extract_random_chance_mAPs()
#    plt.plot(plt.xlim(), [random_chance_mAP, random_chance_mAP], color='k', linestyle='dotted', label='chance')
    plt.ylim((45,70))
    plt.xlabel('epoch')
    plt.ylabel('mAP (%)')
    plt.title('testing mAP of DualCoOp++ with pseudolabeling')
    plot_filename = os.path.join(PLOT_DIR, '11_21_23_adjust_logits_0-plot.png')
    legend_filename = os.path.join(PLOT_DIR, '11_21_23_adjust_logits_0-LEGEND.png')
...
if __name__ == '__main__':
    plot_results_11_21_23_adjust_logits_0()
