...
def mAP(targs, preds, report_individual_classes=False):
...
@EVALUATOR_REGISTRY.register()
class MLClassificationDualCoOpStarStar(EvaluatorBase):
    """Evaluator for classification."""

    def __init__(self, cfg, lab2cname=None, **kwargs):
        super().__init__(cfg)
        self.is_for_dualcoopstarstar = True
        self._lab2cname = lab2cname
        self._y_true = []
        self._y_pred_dict = {} #each type of inference algo will map to a list of predictions
        self.activate_func = cfg.TEST.EVALUATOR_ACT
        self.cfg = cfg

    def reset(self):
        self._y_true = []
        self._y_pred_dict = {k : [] for k in sorted(self._y_pred_dict)}

    def process(self, logits_dict, gt):
        self._y_true.append(gt.data)
        if self._y_pred_dict == {}:
            for k in sorted(logits_dict.keys()):
                self._y_pred_dict[k] = []

        assert(sorted(logits_dict.keys()) == sorted(self._y_pred_dict.keys()))
        for k in sorted(self._y_pred_dict.keys()):
            self._y_pred_dict[k].append(logits_dict[k])

    def evaluate(self, report_individual_classes=False):
        targets = torch.cat(self._y_true, dim=0).cpu().numpy()
        results_all = {}
        if report_individual_classes:
            results_all_individual_classes = {}

        for k in sorted(self._y_pred_dict.keys()):
            preds = torch.cat(self._y_pred_dict[k], dim=0).cpu().numpy()
            if report_individual_classes:
                mAP_score, mAP_scores_individual_classes = mAP(targets, preds, report_individual_classes=True)
            else:
                mAP_score = mAP(targets, preds, report_individual_classes=False)

            results_all[k] = mAP_score
            print("mAP score (%s):"%(k), mAP_score)
            if report_individual_classes:
                results_all_individual_classes[k] = mAP_scores_individual_classes

        results = OrderedDict()
        results['mAP'] = results_all[self.activate_func]
        results['results_all'] = results_all
        if report_individual_classes:
            results['results_all_individual_classes'] = results_all_individual_classes

        if self.cfg.TEST.SAVE_PREDS:
            with open(self.cfg.TEST.SAVE_PREDS, 'wb') as f:
                pickle.dump((targets, {k : torch.cat(self._y_pred_dict[k], dim=0).cpu().numpy() for k in sorted(self._y_pred_dict.keys())}), f)
