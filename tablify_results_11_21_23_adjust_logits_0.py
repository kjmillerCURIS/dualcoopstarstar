...
from result_utils_11_21_23 import extract_mAPs_given_hparams, extract_mAPs_given_runID, extract_zsclip_mAPs
...
def tablify_results_11_21_23_adjust_logits_0():
    rows = []
    rows.append(['algo_type','adjust_logits','use_bias','bandwidth','stepsize','last_mAP','last_epoch','best_mAP','best_epoch'])
    ret_zsclip = extract_zsclip_mAPs()
    zsclip_row = ['ZSCLIP (ensemble)','NA','NA','NA','NA','NA','NA','%.1f'%(ret_zsclip),'NA']
    rows.append(zsclip_row)
    ...
    f = open(os.path.join(TABLE_DIR, '11_21_23_adjust_logits_0-table.csv'), 'w')
    for row in rows:
        f.write(','.join(row) + '\n')
...
if __name__ == '__main__':
    tablify_results_11_21_23_adjust_logits_0()
